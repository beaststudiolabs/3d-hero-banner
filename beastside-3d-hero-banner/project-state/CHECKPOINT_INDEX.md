...
|---|---|---|---|---|---|
| 20260211-010734-status-snapshot-documentation | 2026-02-11T01:07:34Z | status-snapshot-documentation | incremental-change | Added consolidated `CURRENT_STATUS.md` documentation snapshot capturing branch/commit/version status, progress totals, milestone coverage, and immediate QA focus. | no |
| 20260210-145332-direct-pick-axis-gizmo-model3-fix | 2026-02-10T14:53:32Z | direct-pick-axis-gizmo-model3-fix | incremental-change | Fixed Model 3 drag reliability by preserving `models[].slot` identity, added direct marker click target selection with dropdown sync fallback, improved RGB axis-constrained gizmo behavior with axis labels, and auto-aligned 3D grid/axes to model floor baseline; bumped plugin version to 0.2.9. | no |
| 20260210-135027-model-visibility-transformcontrols-hotfix | 2026-02-10T13:50:27Z | model-visibility-transformcontrols-hotfix | incremental-change | Fixed TransformControls compatibility regression that blocked model visibility by using class-based inheritance and guarded runtime init fallback; bumped plugin version to 0.2.8 for cache-busting. | no |
| 20260210-122522-ambient-gizmo-grid-model-drag-v1 | 2026-02-10T12:25:22Z | ambient-gizmo-grid-model-drag-v1 | incremental-change | Upgraded to scene schema v3 with ambient position anchor, added admin-preview TransformControls drag for ambient/point/model targets, and introduced grid/axes/selected XYZ overlay toggles (admin-only). | no |
